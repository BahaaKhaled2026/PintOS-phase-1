...
struct thread
  {
	...
	/* Owned by thread.c. */
	struct list_elem sleep_elem;         /* List element for sleeping
	                                       threads. */
	int64_t waking_time;  /* Ticks remaining from waking up. */
	
	/* Owned by thread.c and synch.c. */
	int actual_priority;                  /* Real priority while being
	                                       donated. */
	...
	int nice;                           /* Determines how nice a thread should
	                                       be to other threads. */
	fixed_point recent_cpu;                 /* The recent cpu. */
  };
...
void thread_sort (struct thread *);
