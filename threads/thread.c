...
static void pri_mlfqs(struct thread *);
...
{
  enum intr_level old_level = intr_disable();
  int actual_priority = t->actual_priority;
  if (list_empty(&t->locks_held))
    t->priority = actual_priority;
  else
  {
    ...
    t->priority = actual_priority > lock_priority ? actual_priority : lock_priority;
  }
  intr_set_level(old_level);
}
...
void thread_sort(struct thread *t)
{
...
void update_recent_CPU(struct thread *t, void *aux UNUSED)
{
  t->recent_cpu = FP_ADD_MIX(
      FP_DIV(FP_MULT(FP_MULT_MIX(load_avg, 2), t->recent_cpu),FP_ADD_MIX(FP_MULT_MIX(load_avg, 2), 1)),t->nice);
  pri_mlfqs(t);
}
